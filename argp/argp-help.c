...
static
const char *dgettext_safe(const char *d, const char *m)
{
        ...
        argp_fmtstream_t stream)
{
    ...
    if (child)
        while (child->argp && !(first_only && anything))
            anything |= argp_doc((child++)->argp, state,
...
const char *__argp_short_program_name(void)
{
    return getprogname();
}
        ...
        va_list ap, unsigned int mode_flags)
{
    if (!state || !(state->flags & ARGP_NO_ERRS)) {
        ...
        if (stream) {
            fputs(state ? state->name : __argp_short_program_name(), stream);
            putc(':', stream);
            putc(' ', stream);
